...
def create_model(
    ...
    if base_model == "resnet":
        ...
        print("Using resnet18 with 3 input channels")
        model = resnet18(pretrained=True)
        if init_classifier:
            model.fc = nn.Linear(model.fc.in_features, num_classes)
    elif base_model == "resnet18_custom":
        print("Using resnet18 with custom input channels")
        model = resnet18(pretrained=False)
        model.conv1 = nn.Conv2d(in_channels=num_channel, out_channels=model.conv1.out_channels, kernel_size=model.conv1.kernel_size, stride=model.conv1.stride, padding=model.conv1.padding, bias=model.conv1.bias)
        model.fc = nn.Linear(model.fc.in_features, num_classes)
